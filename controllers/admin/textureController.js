...
exports.getAllTextures = async (req, res) => {
  ...
  exports.createTexture = async (req, res) => {
    ...
    if (!textureCategory || !req.file) {
     ...
     const existingTexture = await Texture.findOne({textureCategory});
      if(existingTexture){
        console.log("existingTexture:", existingTexture);
        
        return res.status(400).json({success:false, message:"Texture category already exist"})
      }
    ...
    try {
      ...
      res.status(201).json({ success: true, message: "Texture added successfully" });
    } catch (err) {
        console.log("err", );
